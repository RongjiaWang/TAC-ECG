...
class adapter1(torch.nn.Module):
    def __init__(self, in_channel, out_channel):
        super(adapter1, self).__init__()
        self.conv1 = torch.nn.Conv1d(in_channels=in_channel, out_channels=8, kernel_size=1, stride=1,
                                     padding='same', bias=False)
        self.relu = torch.nn.ReLU(inplace=True)
        self.conv2 = torch.nn.Conv1d(in_channels=8, out_channels=out_channel, kernel_size=1, stride=1,
                                     padding='same', bias=False)
...
class ConvLayer(nn.Sequential):
    ...
    def __init__(self, ni, nf, ks=3, stride=1, padding=None, bias=None, norm='bn', bn_1st=True,
        ...
        if ks == 3:
            adapter = adapter1(in_channel=nf, out_channel=nf)
            act_bn.append(adapter)
